...
internal class Program
{
    static Task<int> Main(string[] args)
    {
        ...
        //===============
        {
            var keyArgument = new CliArgument<string>(name: "signkey") { Description = "Use custom key used sign the jwt." };

            var encodekeyCommand = new CliCommand("encode-key", "Encode a key to Base64 to be used in application settings.")
            {
                keyArgument,
            };
            rootCommand.Subcommands.Add(encodekeyCommand);

            encodekeyCommand.Action = CommandHandler.Create(
                static async (EncodeKeyOptions o) =>
                {
                    //create token builder
                    var keyBuf = TokenGenerator.GetSigningKeyBufferFromString(o.SigningKey);

                    //write encoded key
                    Console.WriteLine(Convert.ToBase64String(keyBuf));
                });
        }
    ...
    public class EncodeKeyOptions
    {
        public string SigningKey { get; set; }

        public EncodeKeyOptions(string signkey)
        {
            this.SigningKey = signkey;
        }
    }
